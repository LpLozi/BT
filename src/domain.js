import{normalizeClientData,validDate}from'./clients-domain.js';
...
export const clampDay=n=>Math.max(1,Math.min(28,Math.floor(Number(n))||25));
...
export function periodRange(y,m,settings){
 const next=new Date(y,m+1,1),nextY=next.getFullYear(),nextM=next.getMonth();
 const start=isoLocal(new Date(y,m,clampDay(settings.overrides?.[`${y}-${m}`]??settings.periodDay)));
 const end=addDays(isoLocal(new Date(nextY,nextM,clampDay(settings.overrides?.[`${nextY}-${nextM}`]??settings.periodDay))),-1);
 return [start,end];
}
export function periodFor(ds,settings){const [y,m]=ds.split('-').map(Number);const start=periodRange(y,m-1,settings)[0];const base=new Date(y,m-1-(ds<start?1:0),1);return {y:base.getFullYear(),m:base.getMonth()};}
...
export function expandRec(rec){
 const out=[];(rec||[]).forEach(r=>{if(!r.start)return;const cutoff=r.active===false?(r.pausedAt||today()):null;const count=Math.min(240,Math.max(0,Math.floor(Number(r.count)||24)));for(let c=0;c<count;c++){const d=addMonths(r.start,c);if(cutoff&&d>cutoff)break;out.push({id:`${r.id}-${c}`,parent:r.id,type:r.type,cat:r.cat,acc:r.acc,amount:Number(r.amount),date:d,note:`${r.note||'Tekrarlayan'} · otomatik`,auto:true,rec:true})}});return out
}
...
export function carryCalc(tx,y,m,data){
 let min=periodFor(today(),data.settings); tx.forEach(t=>{const p=periodFor(t.date,data.settings);if(p.y*12+p.m<min.y*12+min.m)min=p});let c=0;
 for(let yy=min.y,mm=min.m;yy*12+mm<y*12+m;mm++){if(mm>11){mm=0;yy++}const [s,e]=periodRange(yy,mm,data.settings),l=tx.filter(t=>t.date>=s&&t.date<=e);c=Math.max(0,c+sum(l,'gelir')-sum(l,'gider'))}return c
}
...
export function makeInstallment(form){
 const total=Number(String(form.amount).replace(',','.')),count=Math.min(240,Math.max(2,Math.floor(Number(form.count)||2)));const cents=Math.round(total*100),base=Math.floor(cents/count),items=[];let used=0;
...
export function isReferenced(data,kind,id){
 if(kind==='cat')return [...data.tx,...data.installments,...data.recurring,...data.trash.map(x=>x.payload).filter(Boolean)].some(x=>x.cat===id);
 if(kind==='acc')return [...data.tx,...data.installments,...data.recurring,...(data.trash||[]).map(x=>x.payload).filter(Boolean)].some(x=>x.acc===id);return false
}
...
export function normalizeData(raw){
 for(const key of ['tx','cats','accounts','installments','recurring','trash']){
  if(raw?.[key]!=null&&(!Array.isArray(raw[key])||raw[key].some(x=>!x||typeof x!=='object')))throw new Error('Yedekte geçersiz kayıtlar var. Mevcut veriler korundu.');
 }
 if(raw?.tx?.some(t=>!t.id||!validDate(t.date)||!['gelir','gider'].includes(t.type)||!Number.isFinite(Number(t.amount))))throw new Error('İşlem verileri geçersiz. Mevcut veriler korundu.');
 ...
 return{...d,...(d.clientTracker?{clientTracker:normalizeClientData(d.clientTracker)}:{}),schemaVersion:2,tx:Array.isArray(d.tx)?d.tx:[],cats,accounts,settings:{periodDay:clampDay(d.settings?.periodDay),overrides:{...(d.settings?.overrides||{})}},installments:Array.isArray(d.installments)?d.installments.map(i=>({...i,status:i.status||'active',items:Array.isArray(i.items)?i.items:[]})):[],recurring:Array.isArray(d.recurring)?d.recurring.map(r=>({...r,active:r.active!==false,...(r.active===false?{pausedAt:r.pausedAt||today()}:{} )})):[],budgets:d.budgets&&typeof d.budgets==='object'?d.budgets:{},trash:purgeExpiredTrash(Array.isArray(d.trash)?d.trash:[]),prefs:{...defaultData.prefs,...(d.prefs||{})}}
}
